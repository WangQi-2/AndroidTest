...
public class RatingProgress extends View {
    ...
    private static String TAG = RatingProgress.class.getSimpleName();
    // 前景图星星
    ...
    public RatingProgress(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.RatingProgress);
        Drawable mForegoundDrawable = a
                .getDrawable(R.styleable.RatingProgress_drawable_foreground);
        ...
        Drawable mBackgoundDrawable = a
                .getDrawable(R.styleable.RatingProgress_drawable_background);
        mBBmp = ((BitmapDrawable) mBackgoundDrawable).getBitmap();
        mDrawableSpacing = a.getDimensionPixelSize(
                R.styleable.RatingProgress_drawable_spacing, 0);
        mCount = a.getInteger(R.styleable.RatingProgress_drawable_count, 0);
        mMaxProgress = a.getInteger(R.styleable.RatingProgress_drawable_max_progress, 0);
        mCurProgress = a.getInteger(R.styleable.RatingProgress_drawable_cur_progress, 0);
    ...
    private Bitmap generateProgressBar() {
        if (mCurProgress == 0) {
            return mBackgroundBar;
        }
        if (mCurProgress >= mMaxProgress) {
            return mForegroundBar;
        }
